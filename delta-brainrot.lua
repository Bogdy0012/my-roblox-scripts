-- SCRIPT FINAL - STEAL A BRAINROT (DELTA + XENO)
-- SUNET OPRIT COMPLET + TOATE FUNCȚIILE
-- Funcționează pe Delta și Xeno
...
-- ===== DISCORD (compatibil cu Xeno și Delta) =====
...
-- ===== OPREȘTE COMPLET SUNETUL (metoda care merge) =====
local function killAllAudio()
    print("🔇 Oprire completă sunet...")
    
    -- 1. Volum global
    pcall(function()
        SoundService.Volume = 0
        
        -- Setează toate canalele audio la 0
        for i = 1, 10 do
            pcall(function()
                SoundService:SetVolume(i, 0)
            end)
        end
    end)
    
    -- 2. Setări Roblox (UserSettings)
    pcall(function()
    ...
    -- 3. Oprește toate sunetele existente
    pcall(function()
